...
check_new_version () {
    ...
    if ping -q -c 1 -W 1 google.com >/dev/null; then
        checked_version=$(curl -s https://raw.githubusercontent.com/Athexhacker/ANDRO-EYE/master/modules/version)
...
if [ -d $PWD/.temp/ ]
    then
        ...
        echo -e "PLEASE INSTALL THE TOOL IF NOT INSTALLED OR IF INSTALLED MAKE A '.temp' DIRECTORY FIRST. ( BY EXECUTING mkdir .temp )" && exit
fi
...
if [ "$?" == 0 ]; then
    ...
    echo -e "\nADB IS NOT INSTALLED, RUN THE INSTALLATION SCRIPT OR TRY INSTALLING 'ADB' MANUALLY ( BY EXECUTING sudo apt install adb)"
    exit 1
fi
...
if [ "$?" == 0 ]; then
    ...
    echo -e "\nFASTBOOT IS NOT INSTALLED, RUN THE INSTALLATION SCRIPT OR TRY INSTALLING 'FASTBOOT' MANUALLY. ( BY EXECUTING sudo apt install fastboot )"
    exit 1
fi
...
\e[0m"
echo
echo -e "\e[93mDEVELOPED BY \e[1;91mA T H E X\e[0m"
}
